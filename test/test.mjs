...
must(1234, 'CONSEC_QUAD_EXACT'); must(1234, 'ASCENSION'); must(1234, 'FOUR_DIGITS');
...
mustNot(1, 'POWER_OF_TWO'); // 1 not counted as a power of 2 (exponent >= 1)
...
if (epOf(1, 'NINETEENTH_POWER') !== 33333367) throw new Error('1: top power tier should score');
...
must(1213, 'CONSEC_PAIR_EXACT'); must(1213, 'CONSEC_PAIR_ADJACENT');
...
if (epOf(0, 'NINETEENTH_POWER') !== 33333367) throw new Error('0: top power tier should score');
...
if (epOf(407777, 'TRIPS') !== 0) throw new Error('Three of a Kind superseded by Four of a Kind');
...
// Pair still scores normally when there's no contiguous pair.
if (compute(6161).badges.find(b => b.id === 'PAIR').ep !== 120) throw new Error('6161 Pair should score 120');
